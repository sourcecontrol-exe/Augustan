...
async def run_paper_trading_demo():
    ...
    try:
        ...
        # Buy BTC
        btc_order = paper_trader.place_market_order('BTC/USDT', paper_trader.OrderSide.BUY, 0.01)
        if btc_order:
            logger.info(f"Placed BTC buy order: {btc_order}")
        
        # Buy ETH
        eth_order = paper_trader.place_market_order('ETH/USDT', paper_trader.OrderSide.BUY, 0.1)
        if eth_order:
...
def run_quick_demo():
    ...
    try:
        ...
        order_id = paper_trader.place_market_order('BTC/USDT', paper_trader.OrderSide.BUY, 0.001)
