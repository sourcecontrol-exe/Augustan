...
class DataLoader:
    ...
    def load_data(self, symbol: str, source: str = 'ccxt', **kwargs) -> pd.DataFrame:
        """
        Load historical data from specified source
        
        Args:
            symbol: Trading symbol (e.g., 'BTC/USDT' for exchanges, 'BTC-USD' for yfinance)
            source: Data source ('ccxt', 'binance', 'yfinance', 'csv')
            **kwargs: Additional parameters for data loading
            
        Returns:
            DataFrame with OHLCV data
        """
        if source in ['ccxt', 'binance', 'okx', 'bybit', 'kucoin']:
            return self._load_ccxt_data(symbol, source, **kwargs)
    ...
    def _load_ccxt_data(self, symbol: str, exchange: str = 'binance',
                       timeframe: str = '1h', limit: int = 1000, **kwargs) -> pd.DataFrame:
        """Load data directly from crypto exchanges using CCXT"""
        if not CCXT_AVAILABLE:
            raise ImportError("CCXT not available. Install with: pip install ccxt")
        
        logger.info(f"Loading {symbol} from {exchange} via CCXT ({timeframe}, {limit} bars)")
        ...
        exchange_id = exchange_mapping.get(exchange, 'binance')
        ...
        try:
            ...
            exchange_instance = exchange_class({
                ...
                'sandbox': kwargs.get('testnet', True),  # Use testnet by default
            ...
            if not ohlcv:
                raise ValueError(f"No data found for {symbol} on {exchange_id}")
            ...
            # Remove timestamp column and ensure proper data types
            df = df.astype(float)
            ...
            logger.error(f"Error loading data from {exchange_id}: {e}")
            raise
    ...
    def get_sample_data(self, symbol: str = 'BTC/USDT', bars: int = 1000) -> pd.DataFrame:
        """Get sample crypto data for testing (defaults to BTC from Binance via CCXT)"""
        try:
            # Try CCXT first for real crypto data
            return self._load_ccxt_data(symbol, 'binance', limit=bars)
        except:
            try:
                # Fallback to yfinance
                yf_symbol = symbol.replace('/', '-').replace('USDT', 'USD')
                return self._load_yfinance(yf_symbol, period='2y', interval='1d')
            except:
...
# Convenience functions for crypto data
def load_crypto_data(symbol: str, exchange: str = 'binance', 
                    timeframe: str = '1h', limit: int = 1000) -> pd.DataFrame:
    """Quick function to load crypto data from exchange via CCXT"""
    loader = DataLoader()
...
def get_sample_btc_data(bars: int = 1000) -> pd.DataFrame:
    """Get sample BTC data for testing from Binance"""
    loader = DataLoader()
    return loader.get_sample_data('BTC/USDT', bars)

def get_crypto_pairs() -> list:
    """Get popular crypto trading pairs"""
    return [
